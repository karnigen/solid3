...
DT_ShapeHandle createComplex()
{
    ...
    for (int i0 = -GRID_SCALE; i0 != GRID_SCALE; ++i0)
	{
        for (int j0 = -GRID_SCALE; j0 != GRID_SCALE; ++j0)
		{
			...
			MT_Vector3 verts[4];
			verts[0].setValue(GRID_UNIT * i0, MT_Scalar(.25) * (i0*i0 + j0*j0), GRID_UNIT * j0);
            verts[1].setValue(GRID_UNIT * i0, MT_Scalar(.25) * (i0*i0 + j1*j1), GRID_UNIT * j1);
            verts[2].setValue(GRID_UNIT * i1, MT_Scalar(.25) * (i1*i1 + j1*j1), GRID_UNIT * j0);
            verts[3].setValue(GRID_UNIT * i1, MT_Scalar(.25) * (i1*i1 + j0*j0), GRID_UNIT * j1);
#ifdef USE_QUADS
            DT_Begin();
            DT_Vertex(verts[0]);
            DT_Vertex(verts[1]);
...
static MT_Point3 eye(MT_Scalar(0.0), MT_Scalar(0.0), DISTANCE);
